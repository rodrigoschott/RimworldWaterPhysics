...
public static class Toils_Goto
{
	...
	public static Toil GotoBuild(TargetIndex ind)
	{
		...
		toil.initAction = delegate
		{
			...
			LocalTargetInfo target = actor2.jobs.curJob.GetTarget(ind);
			Thing thing2 = target.Thing;
			if (RCellFinder.TryFindGoodAdjacentSpotToTouch(actor2, thing2, out var result))
			{
				actor2.pather.StartPath(result, PathEndMode.OnCell);
			}
			else
			{
				actor2.pather.StartPath(target, PathEndMode.OnCell);
			}
		};
