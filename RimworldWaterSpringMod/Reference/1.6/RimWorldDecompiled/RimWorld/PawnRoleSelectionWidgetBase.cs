	...
	private void DrawPawnListInternal(Rect viewRect, Rect listRect)
	{
		...
		try
		{
			...
			}, DropContextNotParticipating, null, locked: false, null, delegate(Pawn p)
			{
				RoleType firstRole3;
				bool mustReplace3;
				string text5 = CannotAssignReason(p, null, out firstRole3, out mustReplace3);
				if (text5 != null)
				{
			...
			}, delegate(Pawn p)
			{
				...
				{
					IGrouping<string, RoleType> localRoleGroup = item3;
					RoleType val2 = item3.First();
					text4 = CannotAssignReason(p, localRoleGroup, out firstRole2, out var mustReplace, isReplacing: true);
					Pawn replacing = (mustReplace ? localRoleGroup.SelectMany((RoleType role) => assignments.AssignedPawns(role)).Last() : null);
					Action action2 = ((text4 != null) ? null : ((Action)delegate
					{
						if (mustReplace)
						{
							TryAssignReplace(p, localRoleGroup, replacing);
						}
						else
						{
							TryAssign(p, localRoleGroup);
						}
					}));
