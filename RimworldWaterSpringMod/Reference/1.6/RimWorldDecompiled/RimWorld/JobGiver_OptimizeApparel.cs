...
public class JobGiver_OptimizeApparel : ThinkNode_JobGiver
{
	...
	protected override Job TryGiveJob(Pawn pawn)
	{
		...
		{
			Apparel apparel = (Apparel)tmpApparelList[j];
			if (!currentApparelPolicy.filter.Allows(apparel) || !apparel.IsInAnyStorage() || apparel.IsForbidden(pawn) || apparel.IsBurning() || (apparel.def.apparel.gender != 0 && apparel.def.apparel.gender != pawn.gender))
			{
				continue;
			}
			...
			if (DebugViewSettings.debugApparelOptimize)
			{
				debugSb.AppendLine($"{apparel.LabelCap}: {num3:F2}");
			}
			if (num3 < 0.05f || num3 < num2 || (CompBiocodable.IsBiocoded(apparel) && !CompBiocodable.IsBiocodedFor(apparel, pawn)) || !ApparelUtility.HasPartsToWear(pawn, apparel.def))
			{
				continue;
			}
			LocalTargetInfo target = apparel;
			if (apparel.ParentHolder is IApparelSource apparelSource && apparelSource is Thing thing2)
			{
			...
			if (pawn.CanReserveAndReach(target, PathEndMode.OnCell, pawn.NormalMaxDanger()) && apparel.def.apparel.developmentalStageFilter.Has(pawn.DevelopmentalStage))
			{
				thing = apparel;
				num2 = num3;
			}
