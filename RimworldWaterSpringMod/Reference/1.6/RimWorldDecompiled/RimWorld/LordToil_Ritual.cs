		...
		for (int i = 0; i < tmpPawns.Count; i++)
		{
			if (!hashSet.Contains(tmpPawns[i]))
			{
				if (i != num)
				{
					...
					Pawn pawn4 = (list[index] = pawn);
					pawn4 = (list2[index2] = pawn2);
					...
					CachedPawnRitualDuty cachedPawnRitualDuty4 = (list3[index2] = cachedPawnRitualDuty);
					cachedPawnRitualDuty4 = (list4[index] = cachedPawnRitualDuty2);
				}
				num++;
			}
		}
