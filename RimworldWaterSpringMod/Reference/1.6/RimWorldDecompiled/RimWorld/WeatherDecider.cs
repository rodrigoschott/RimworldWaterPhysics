...
public class WeatherDecider : IExposable
{
	...
	private float CurrentWeatherCommonality(WeatherDef weather)
	{
		...
		for (int i = 0; i < biome.baseWeatherCommonalities.Count; i++)
		{
			...
			{
				num *= activeCondition.WeatherCommonalityFactor(weather, map);
			}
			commonality = num;
			break;
		}
		PlanetTile tile = map.Tile;
		for (int j = 0; j < map.TileInfo.Mutators.Count; j++)
		{
			map.TileInfo.Mutators[j].Worker?.MutateWeatherCommonalityFor(weather, tile, ref commonality);
		}
		return commonality;
	}
