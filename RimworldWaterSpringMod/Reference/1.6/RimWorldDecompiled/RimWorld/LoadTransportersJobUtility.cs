...
public static class LoadTransportersJobUtility
{
	...
	public static ThingCount FindThingToLoad(Pawn p, CompTransporter transporter)
	{
		...
		if (thing == null)
		{
			...
			{
				if (neededThing is Pawn { Spawned: not false } pawn && ((!pawn.IsFreeColonist && !pawn.IsColonyMech) || pawn.Downed || (pawn.needs?.energy?.IsLowEnergySelfShutdown).GetValueOrDefault()) && !pawn.inventory.UnloadEverything && p.CanReserveAndReach(pawn, PathEndMode.Touch, Danger.Deadly))
				{
					neededThings.Clear();
					tmpAlreadyLoading.Clear();
