...
public class JobDriver_InteractThing : JobDriver
{
	...
	private Toil WaitForActivate(int remainingTicks, int totalTicks)
	{
		...
		{
			Toil toil4 = toil;
			toil4.tickIntervalAction = (Action<int>)Delegate.Combine(toil4.tickIntervalAction, (Action<int>)delegate(int delta)
			{
				Interactable.progress = 1f - (float)ticksLeftThisToil / (float)totalTicks * (float)delta;
			});
		}
