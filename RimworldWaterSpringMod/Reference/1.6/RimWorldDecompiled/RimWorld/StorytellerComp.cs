...
public abstract class StorytellerComp
{
	...
	public virtual void DebugTablesIncidentChances()
	{
		...
		{
			IIncidentTarget defaultTarget2 = GetDefaultTarget(d);
			if (defaultTarget2 == null)
			{
				return false;
			}
			IncidentParms parms = StorytellerUtility.DefaultParmsNow(d.category, defaultTarget2);
		...
		{
			IIncidentTarget defaultTarget = GetDefaultTarget(def);
			if (defaultTarget == null)
			{
				return 1f;
			}
			for (int i = 0; i < defaultTarget.StoryState.RecentRandomIncidents.Count; i++)
			{
				if (defaultTarget.StoryState.RecentRandomIncidents[i] == def)
				{
