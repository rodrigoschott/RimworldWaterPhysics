...
public class Pawn_AbilityTracker : IExposable
{
	...
	public List<Ability> AICastableAbilities(LocalTargetInfo target, bool offensive)
	{
		...
		foreach (Ability item in AllAbilitiesForReading)
		{
			...
			if (pawn.IsAnimal)
			{
				...
				if (faction != null && faction.IsPlayer && pawn.RaceProps.specialTrainables != null)
				{
					foreach (TrainableDef specialTrainable in pawn.RaceProps.specialTrainables)
					{
						if (specialTrainable.enablesAbility == item.def)
						{
							Pawn_TrainingTracker training = pawn.training;
							if (training != null && training.HasLearned(specialTrainable))
							{
								flag = false;
								break;
							}
			...
			if (flag && item.def.ai_IsOffensive == offensive && (item.AICanTargetNow(target) || item.AICanTargetNow(pawn)))
			{
	...
	{
		bool visiblePrimary = pawn.IsColonistPlayerControlled || pawn.IsColonyMechPlayerControlled || pawn.IsColonySubhumanPlayerControlled;
		foreach (Ability a in AllAbilitiesForReading)
		{
			if (!visiblePrimary && pawn.training != null && pawn.RaceProps.specialTrainables != null)
			{
				foreach (TrainableDef specialTrainable in pawn.RaceProps.specialTrainables)
				{
					if (specialTrainable.enablesAbility == a.def && pawn.training.HasLearned(specialTrainable))
					{
						visiblePrimary = true;
					}
				}
