...
public class Dialog_BillConfig : Window
{
	...
	private void FillOutputDropdownOptions(ref List<FloatMenuOption> opts, string prefix, Action<ISlotGroup> selected)
	{
		...
		foreach (KeyValuePair<string, List<ISlotGroup>> kvp in tmpGroups.OrderBy((KeyValuePair<string, List<ISlotGroup>> kvp) => (kvp.Value.Count > 0) ? kvp.Value[0].GroupingOrder : 0))
		{
