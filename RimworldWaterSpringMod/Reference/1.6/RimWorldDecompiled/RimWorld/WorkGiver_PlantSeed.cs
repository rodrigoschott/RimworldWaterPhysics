...
public class WorkGiver_PlantSeed : WorkGiver_Scanner
{
	...
	public override bool HasJobOnThing(Pawn pawn, Thing t, bool forced = false)
	{
		...
		{
			if (pawn.CanReach(plantCells[i], PathEndMode.Touch, Danger.Deadly) && (compPlantable.Props.plantDefToSpawn.plant.vacuumResistant || !(plantCells[i].GetVacuum(t.Map) >= 0.5f)))
			{
