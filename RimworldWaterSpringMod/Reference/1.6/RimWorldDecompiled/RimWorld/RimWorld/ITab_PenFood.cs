...
public class ITab_PenFood : ITab_PenBase
{
	...
	private void DrawAnimalPane(ref float curYOuter, float width, float height, PenFoodCalculator calc, Map map)
	{
		...
		DrawCell("PenFoodTab_AnimalType".Translate(), ref x2, cellWidth2, num3, TextAnchor.LowerLeft);
		DrawCell("PenFoodTab_Count".Translate(), ref x2, 100f, num3, TextAnchor.LowerCenter);
		...
		foreach (PenFoodCalculator.PenAnimalInfo info in tmpAnimalInfos)
		{
			...
			DrawIconCell(info.animalDef, ref x3, 53f, num2);
			DrawCell(info.animalDef.LabelCap, ref x3, cellWidth2, num2, TextAnchor.MiddleLeft);
			if (!info.example)
			{
				DrawCell(info.TotalCount.ToString(), ref x3, 100f, num2, TextAnchor.MiddleCenter);
				DrawCell(PenFoodCalculator.NutritionPerDayToString(info.TotalNutritionConsumptionPerDay, withUnits: false), ref x3, 120f, num2, TextAnchor.MiddleCenter);
			}
			else
			{
				...
				DrawExampleAnimalControls(info, ref x3, 27f, num2);
			}
