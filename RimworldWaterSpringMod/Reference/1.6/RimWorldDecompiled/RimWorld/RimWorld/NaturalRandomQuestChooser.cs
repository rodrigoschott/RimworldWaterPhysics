...
public static class NaturalRandomQuestChooser
{
	...
	public static QuestScriptDef ChooseNaturalRandomQuest(float points, IIncidentTarget target)
	{
		...
		{
			return DefDatabase<QuestScriptDef>.AllDefs.Where((QuestScriptDef x) => x.IsRootRandomSelected && x.rootIncreasesPopulation == incPop && x.CanRun(points, target)).TryRandomElementByWeight((QuestScriptDef x) => GetNaturalRandomSelectionWeight(x, points, target.StoryState), out chosen);
		}
	}
