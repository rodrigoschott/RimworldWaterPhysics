...
public static class IdeoUIUtility
{
	...
	private static void AddPrecept(Ideo ideo, IdeoEditMode editMode, Func<PreceptDef, bool> filter, bool group)
	{
		...
		List<FloatMenuOption> list = DefDatabase<PreceptDef>.AllDefs.Where(filter).ToList();
		bool flag = GenCollection.Any(list, (PreceptDef p) => p.preceptClass == typeof(Precept_Ritual) && (showAll || p.visible) && (bool)CanListPrecept(ideo, p, editMode));
		...
		{
			if (!thingGroupsTmp.TryGetValue(thing, out list))
			{
				thingGroupsTmp[thing] = (list = new List<FloatMenuOption>());
