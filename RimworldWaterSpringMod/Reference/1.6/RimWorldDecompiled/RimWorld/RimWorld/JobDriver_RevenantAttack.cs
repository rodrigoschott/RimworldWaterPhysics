...
public class JobDriver_RevenantAttack : JobDriver
{
	...
	protected override IEnumerable<Toil> MakeNewToils()
	{
		...
		Toil stalk = Toils_Combat.FollowAndMeleeAttack(TargetIndex.A, TargetIndex.B, delegate
		{
			...
			{
				Pawn pawn4 = (Pawn)job.GetTarget(TargetIndex.A).Thing;
				JobDriver curDriver = pawn.jobs.curDriver;
				Find.Anomaly.Hypnotize(pawn4, pawn, LongStunTicks);
		...
		toil2.tickIntervalAction = (Action<int>)Delegate.Combine(toil2.tickIntervalAction, (Action<int>)delegate(int delta)
		{
			...
			{
				Pawn pawn3 = curJob.targetA.Pawn;
				if (pawn.Position.InHorDistOf(pawn3.Position, RevealRange))
				{
		...
		{
			Pawn pawn2 = job.GetTarget(TargetIndex.A).Pawn;
