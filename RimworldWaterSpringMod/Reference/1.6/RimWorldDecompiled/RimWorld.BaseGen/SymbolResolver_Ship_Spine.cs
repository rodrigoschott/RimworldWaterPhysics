...
public class SymbolResolver_Ship_Spine : SymbolResolver
{
	public override void Resolve(ResolveParams rp)
	{
		...
		while (true)
		{
			...
			if (!allowPlacementOffEdge.HasValue || allowPlacementOffEdge.GetValueOrDefault() || (intVec - toIntVec).ToIntVec3.GetThingList(map).Any((Thing thing) => thing.def == ThingDefOf.Ship_Beam))
			{
				break;
			}
			if (num == 20)
		...
		if (!((intVec - intVec2).Magnitude < (float)ship_Beam.Size.z))
		{
			...
			bool? allowPlacementOffEdge = rp.allowPlacementOffEdge;
			if (!allowPlacementOffEdge.HasValue || allowPlacementOffEdge.GetValueOrDefault() || Rand.Value < 0.3f)
			{
