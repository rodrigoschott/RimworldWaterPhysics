...
public class QuestPart_ActivateStructuresAlert : QuestPart_Alert
{
	...
	public override string AlertLabel => string.Concat("StructuresActivatedAlertLabel".Translate() + ": ", structures.Count((Thing s) => (s?.TryGetComp<CompVoidStructure>()?.Active).GetValueOrDefault()).ToString(), "/", structures.Count.ToString());
