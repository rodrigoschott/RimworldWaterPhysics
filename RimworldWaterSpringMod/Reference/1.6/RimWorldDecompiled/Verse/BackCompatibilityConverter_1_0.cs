...
public class BackCompatibilityConverter_1_0 : BackCompatibilityConverter
{
	...
	public override void PostLoadSavegame(string loadingVersion)
	{
		...
		{
			if (worldObjects[i].def == WorldObjectDefOf.EscapeShip && !Find.QuestManager.QuestsListForReading.Any((Quest x) => x.PartsListForReading.Any((QuestPart y) => y is QuestPart_NoWorldObject && ((QuestPart_NoWorldObject)y).worldObject == worldObjects[i])))
			{
				MakeAndAddWorldObjectQuest(worldObjects[i], null);
			}
		}
		for (i = 0; i < worldObjects.Count; i++)
		{
			if (worldObjects[i] is PeaceTalks && !Find.QuestManager.QuestsListForReading.Any((Quest x) => x.PartsListForReading.Any((QuestPart y) => y is QuestPart_NoWorldObject && ((QuestPart_NoWorldObject)y).worldObject == worldObjects[i])))
			{
				Quest quest2 = MakeAndAddWorldObjectQuest(worldObjects[i], null);
				ChoiceLetter choiceLetter2 = Find.Archive.ArchivablesListForReading.OfType<ChoiceLetter>().FirstOrDefault((ChoiceLetter x) => x.lookTargets != null && x.lookTargets.targets.Contains(worldObjects[i]));
