...
public class LoadedObjectDirectory
{
	...
	public void RegisterLoaded(ILoadReferenceable reffable)
	{
		...
		catch (Exception ex5)
		{
			...
			Log.Error("Exception registering " + reffable.GetType()?.ToString() + " " + text5 + " in loaded object directory with unique load ID " + text4 + ": " + ex5);
		}
		...
		catch (Exception ex7)
		{
			...
			Log.Error("Exception registering " + reffable.GetType()?.ToString() + " " + text6 + " in loaded object directory with unique thing ID " + thing.thingIDNumber + ": " + ex7);
		}
	}
