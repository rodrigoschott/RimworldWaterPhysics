	...
	public IEnumerable<Designator> AllIdeoDesignators
	{
		get
		{
			...
			IEnumerable<Designator> GetDesignatorsFromStyleCategory(StyleCategoryDef categoryDef)
			{
				...
				{
					for (int j = 0; j < categoryDef.addDesignators.Count; j++)
					{
						if (categoryDef.addDesignators[j].designationCategory == this)
						{
							yield return GetCachedDesignator(categoryDef.addDesignators[j]);
						}
					}
				}
				if (categoryDef.addDesignatorGroups != null)
				{
					for (int j = 0; j < categoryDef.addDesignatorGroups.Count; j++)
					{
						Designator_Dropdown cachedDropdown = GetCachedDropdown(categoryDef.addDesignatorGroups[j]);
